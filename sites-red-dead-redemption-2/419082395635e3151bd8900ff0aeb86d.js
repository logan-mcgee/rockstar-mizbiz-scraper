! function() {
  try {
    var e = "undefined" != typeof window ? window : "undefined" != typeof global ? global : "undefined" != typeof self ? self : {},
      t = (new Error).stack;
    t && (e._sentryDebugIds = e._sentryDebugIds || {}, e._sentryDebugIds[t] = "850c6325-287d-4962-b58c-e895c8f6860b", e._sentryDebugIdIdentifier = "sentry-dbid-850c6325-287d-4962-b58c-e895c8f6860b")
  } catch (e) {}
}();
var _global2 = "undefined" != typeof window ? window : "undefined" != typeof global ? global : "undefined" != typeof self ? self : {};
_global2._sentryModuleMetadata = _global2._sentryModuleMetadata || {}, _global2._sentryModuleMetadata[(new Error).stack] = {
  release: "sentry-release-id",
  packageName: "@rockstargames/sites-red-dead-redemption-2",
  dsn: "https://45716709f6ae4d08adc015d264f231ae@o432808.ingest.sentry.io/4504565542748160"
};
var _global = "undefined" != typeof window ? window : "undefined" != typeof global ? global : "undefined" != typeof self ? self : {};
_global.SENTRY_RELEASE = {
  id: "sentry-release-id"
}, (self.webpackChunk_rockstargames_sites_red_dead_redemption_2 = self.webpackChunk_rockstargames_sites_red_dead_redemption_2 || []).push([
  [288], {
    40288: (e, t, n) => {
      n.r(t), n.d(t, {
        AbortedDeferredError: () => a.ay,
        Await: () => r.g5,
        BrowserRouter: () => k,
        Form: () => P,
        HashRouter: () => x,
        Link: () => N,
        MemoryRouter: () => r.wF,
        NavLink: () => U,
        Navigate: () => r.YX,
        NavigationType: () => a.So,
        Outlet: () => r.yY,
        Route: () => r.kX,
        Router: () => r.E5,
        RouterProvider: () => C,
        Routes: () => r.c5,
        ScrollRestoration: () => I,
        UNSAFE_DataRouterContext: () => r.Mh,
        UNSAFE_DataRouterStateContext: () => r.Cu,
        UNSAFE_LocationContext: () => r._W,
        UNSAFE_NavigationContext: () => r.yo,
        UNSAFE_RouteContext: () => r.EN,
        UNSAFE_ViewTransitionContext: () => R,
        UNSAFE_useRouteId: () => r.WM,
        UNSAFE_useScrollRestoration: () => $,
        createBrowserRouter: () => y,
        createHashRouter: () => g,
        createMemoryRouter: () => r.Yj,
        createPath: () => a.Ep,
        createRoutesFromChildren: () => r.CF,
        createRoutesFromElements: () => r._y,
        createSearchParams: () => d,
        defer: () => a.Qv,
        generatePath: () => a.Yz,
        isRouteErrorResponse: () => a.e8,
        json: () => a.qS,
        matchPath: () => a.ot,
        matchRoutes: () => a.Cm,
        parsePath: () => a.O8,
        redirect: () => a.ch,
        redirectDocument: () => a.kt,
        renderMatches: () => r.qW,
        resolvePath: () => a.MP,
        unstable_HistoryRouter: () => A,
        unstable_useBlocker: () => r.C5,
        unstable_usePrompt: () => te,
        unstable_useViewTransitionState: () => ne,
        useActionData: () => r.c3,
        useAsyncError: () => r.qG,
        useAsyncValue: () => r.Yp,
        useBeforeUnload: () => ee,
        useFetcher: () => J,
        useFetchers: () => X,
        useFormAction: () => W,
        useHref: () => r.co,
        useInRouterContext: () => r.Iz,
        useLinkClickHandler: () => K,
        useLoaderData: () => r.GY,
        useLocation: () => r.IT,
        useMatch: () => r.SC,
        useMatches: () => r.y8,
        useNavigate: () => r.i6,
        useNavigation: () => r.Yi,
        useNavigationType: () => r.KM,
        useOutlet: () => r.ag,
        useOutletContext: () => r.Mz,
        useParams: () => r.W4,
        useResolvedPath: () => r.oT,
        useRevalidator: () => r.ER,
        useRouteError: () => r.y_,
        useRouteLoaderData: () => r.$m,
        useRoutes: () => r.wh,
        useSearchParams: () => V,
        useSubmit: () => B
      });
      var o = n(51664),
        r = n(79088),
        a = n(23168);

      function i() {
        return i = Object.assign ? Object.assign.bind() : function(e) {
          for (var t = 1; t < arguments.length; t++) {
            var n = arguments[t];
            for (var o in n) Object.prototype.hasOwnProperty.call(n, o) && (e[o] = n[o])
          }
          return e
        }, i.apply(this, arguments)
      }

      function s(e, t) {
        if (null == e) return {};
        var n, o, r = {},
          a = Object.keys(e);
        for (o = 0; o < a.length; o++) n = a[o], t.indexOf(n) >= 0 || (r[n] = e[n]);
        return r
      }
      const l = "get",
        u = "application/x-www-form-urlencoded";

      function c(e) {
        return null != e && "string" == typeof e.tagName
      }

      function d(e) {
        return void 0 === e && (e = ""), new URLSearchParams("string" == typeof e || Array.isArray(e) || e instanceof URLSearchParams ? e : Object.keys(e).reduce(((t, n) => {
          let o = e[n];
          return t.concat(Array.isArray(o) ? o.map((e => [n, e])) : [
            [n, o]
          ])
        }), []))
      }
      let f = null;
      const m = new Set(["application/x-www-form-urlencoded", "multipart/form-data", "text/plain"]);

      function p(e) {
        return null == e || m.has(e) ? e : null
      }

      function v(e, t) {
        let n, o, r, i, s;
        if (c(d = e) && "form" === d.tagName.toLowerCase()) {
          let s = e.getAttribute("action");
          o = s ? (0, a.mc)(s, t) : null, n = e.getAttribute("method") || l, r = p(e.getAttribute("enctype")) || u, i = new FormData(e)
        } else if (function(e) {
            return c(e) && "button" === e.tagName.toLowerCase()
          }(e) || function(e) {
            return c(e) && "input" === e.tagName.toLowerCase()
          }(e) && ("submit" === e.type || "image" === e.type)) {
          let s = e.form;
          if (null == s) throw new Error('Cannot submit a <button> or <input type="submit"> without a <form>');
          let c = e.getAttribute("formaction") || s.getAttribute("action");
          if (o = c ? (0, a.mc)(c, t) : null, n = e.getAttribute("formmethod") || s.getAttribute("method") || l, r = p(e.getAttribute("formenctype")) || p(s.getAttribute("enctype")) || u, i = new FormData(s, e), ! function() {
              if (null === f) try {
                new FormData(document.createElement("form"), 0), f = !1
              } catch (e) {
                f = !0
              }
              return f
            }()) {
            let {
              name: t,
              type: n,
              value: o
            } = e;
            if ("image" === n) {
              let e = t ? t + "." : "";
              i.append(e + "x", "0"), i.append(e + "y", "0")
            } else t && i.append(t, o)
          }
        } else {
          if (c(e)) throw new Error('Cannot submit element that is not <form>, <button>, or <input type="submit|image">');
          n = l, o = null, r = u, s = e
        }
        var d;
        return i && "text/plain" === r && (s = i, i = void 0), {
          action: o,
          method: n.toLowerCase(),
          encType: r,
          formData: i,
          body: s
        }
      }
      const h = ["onClick", "relative", "reloadDocument", "replace", "state", "target", "to", "preventScrollReset", "unstable_viewTransition"],
        w = ["aria-current", "caseSensitive", "className", "end", "style", "to", "unstable_viewTransition", "children"],
        b = ["reloadDocument", "replace", "state", "method", "action", "onSubmit", "submit", "relative", "preventScrollReset", "unstable_viewTransition"];

      function y(e, t) {
        return (0, a.gv)({
          basename: null == t ? void 0 : t.basename,
          future: i({}, null == t ? void 0 : t.future, {
            v7_prependBasename: !0
          }),
          history: (0, a.iU)({
            window: null == t ? void 0 : t.window
          }),
          hydrationData: (null == t ? void 0 : t.hydrationData) || E(),
          routes: e,
          mapRouteProperties: r.i0,
          window: null == t ? void 0 : t.window
        }).initialize()
      }

      function g(e, t) {
        return (0, a.gv)({
          basename: null == t ? void 0 : t.basename,
          future: i({}, null == t ? void 0 : t.future, {
            v7_prependBasename: !0
          }),
          history: (0, a.Yf)({
            window: null == t ? void 0 : t.window
          }),
          hydrationData: (null == t ? void 0 : t.hydrationData) || E(),
          routes: e,
          mapRouteProperties: r.i0,
          window: null == t ? void 0 : t.window
        }).initialize()
      }

      function E() {
        var e;
        let t = null == (e = window) ? void 0 : e.__staticRouterHydrationData;
        return t && t.errors && (t = i({}, t, {
          errors: S(t.errors)
        })), t
      }

      function S(e) {
        if (!e) return null;
        let t = Object.entries(e),
          n = {};
        for (let [e, o] of t)
          if (o && "RouteErrorResponse" === o.__type) n[e] = new a.sZ(o.status, o.statusText, o.data, !0 === o.internal);
          else if (o && "Error" === o.__type) {
          if (o.__subType) {
            let t = window[o.__subType];
            if ("function" == typeof t) try {
              let r = new t(o.message);
              r.stack = "", n[e] = r
            } catch (e) {}
          }
          if (null == n[e]) {
            let t = new Error(o.message);
            t.stack = "", n[e] = t
          }
        } else n[e] = o;
        return n
      }
      const R = o.createContext({
          isTransitioning: !1
        }),
        _ = o.startTransition;
      class T {
        constructor() {
          this.status = "pending", this.promise = new Promise(((e, t) => {
            this.resolve = t => {
              "pending" === this.status && (this.status = "resolved", e(t))
            }, this.reject = e => {
              "pending" === this.status && (this.status = "rejected", t(e))
            }
          }))
        }
      }

      function C(e) {
        let {
          fallbackElement: t,
          router: n,
          future: a
        } = e, [i, s] = o.useState(n.state), [l, u] = o.useState(), [c, d] = o.useState({
          isTransitioning: !1
        }), [f, m] = o.useState(), [p, v] = o.useState(), [h, w] = o.useState(), {
          v7_startTransition: b
        } = a || {}, y = o.useCallback((e => {
          b ? function(e) {
            _ ? _(e) : e()
          }(e) : e()
        }), [b]), g = o.useCallback(((e, t) => {
          let {
            unstable_viewTransitionOpts: o
          } = t;
          o && null != n.window && "function" == typeof n.window.document.startViewTransition ? p && f ? (f.resolve(), p.skipTransition(), w({
            state: e,
            currentLocation: o.currentLocation,
            nextLocation: o.nextLocation
          })) : (u(e), d({
            isTransitioning: !0,
            currentLocation: o.currentLocation,
            nextLocation: o.nextLocation
          })) : y((() => s(e)))
        }), [y, p, f, n.window]);
        o.useLayoutEffect((() => n.subscribe(g)), [n, g]), o.useEffect((() => {
          c.isTransitioning && m(new T)
        }), [c.isTransitioning]), o.useEffect((() => {
          if (f && l && n.window) {
            let e = l,
              t = f.promise,
              o = n.window.document.startViewTransition((async () => {
                y((() => s(e))), await t
              }));
            o.finished.finally((() => {
              m(void 0), v(void 0), u(void 0), d({
                isTransitioning: !1
              })
            })), v(o)
          }
        }), [y, l, f, n.window]), o.useEffect((() => {
          f && l && i.location.key === l.location.key && f.resolve()
        }), [f, p, i.location, l]), o.useEffect((() => {
          !c.isTransitioning && h && (u(h.state), d({
            isTransitioning: !0,
            currentLocation: h.currentLocation,
            nextLocation: h.nextLocation
          }), w(void 0))
        }), [c.isTransitioning, h]);
        let E = o.useMemo((() => ({
            createHref: n.createHref,
            encodeLocation: n.encodeLocation,
            go: e => n.navigate(e),
            push: (e, t, o) => n.navigate(e, {
              state: t,
              preventScrollReset: null == o ? void 0 : o.preventScrollReset
            }),
            replace: (e, t, o) => n.navigate(e, {
              replace: !0,
              state: t,
              preventScrollReset: null == o ? void 0 : o.preventScrollReset
            })
          })), [n]),
          S = n.basename || "/",
          C = o.useMemo((() => ({
            router: n,
            navigator: E,
            static: !1,
            basename: S
          })), [n, E, S]);
        return o.createElement(o.Fragment, null, o.createElement(r.Mh.Provider, {
          value: C
        }, o.createElement(r.Cu.Provider, {
          value: i
        }, o.createElement(R.Provider, {
          value: c
        }, o.createElement(r.E5, {
          basename: S,
          location: i.location,
          navigationType: i.historyAction,
          navigator: E
        }, i.initialized ? o.createElement(L, {
          routes: n.routes,
          state: i
        }) : t)))), null)
      }

      function L(e) {
        let {
          routes: t,
          state: n
        } = e;
        return (0, r.oj)(t, void 0, n)
      }

      function k(e) {
        let {
          basename: t,
          children: n,
          future: i,
          window: s
        } = e, l = o.useRef();
        null == l.current && (l.current = (0, a.iU)({
          window: s,
          v5Compat: !0
        }));
        let u = l.current,
          [c, d] = o.useState({
            action: u.action,
            location: u.location
          }),
          {
            v7_startTransition: f
          } = i || {},
          m = o.useCallback((e => {
            f && _ ? _((() => d(e))) : d(e)
          }), [d, f]);
        return o.useLayoutEffect((() => u.listen(m)), [u, m]), o.createElement(r.E5, {
          basename: t,
          children: n,
          location: c.location,
          navigationType: c.action,
          navigator: u
        })
      }

      function x(e) {
        let {
          basename: t,
          children: n,
          future: i,
          window: s
        } = e, l = o.useRef();
        null == l.current && (l.current = (0, a.Yf)({
          window: s,
          v5Compat: !0
        }));
        let u = l.current,
          [c, d] = o.useState({
            action: u.action,
            location: u.location
          }),
          {
            v7_startTransition: f
          } = i || {},
          m = o.useCallback((e => {
            f && _ ? _((() => d(e))) : d(e)
          }), [d, f]);
        return o.useLayoutEffect((() => u.listen(m)), [u, m]), o.createElement(r.E5, {
          basename: t,
          children: n,
          location: c.location,
          navigationType: c.action,
          navigator: u
        })
      }

      function A(e) {
        let {
          basename: t,
          children: n,
          future: a,
          history: i
        } = e, [s, l] = o.useState({
          action: i.action,
          location: i.location
        }), {
          v7_startTransition: u
        } = a || {}, c = o.useCallback((e => {
          u && _ ? _((() => l(e))) : l(e)
        }), [l, u]);
        return o.useLayoutEffect((() => i.listen(c)), [i, c]), o.createElement(r.E5, {
          basename: t,
          children: n,
          location: s.location,
          navigationType: s.action,
          navigator: i
        })
      }
      const F = "undefined" != typeof window && void 0 !== window.document && void 0 !== window.document.createElement,
        D = /^(?:[a-z][a-z0-9+.-]*:|\/\/)/i,
        N = o.forwardRef((function(e, t) {
          let n, {
              onClick: l,
              relative: u,
              reloadDocument: c,
              replace: d,
              state: f,
              target: m,
              to: p,
              preventScrollReset: v,
              unstable_viewTransition: w
            } = e,
            b = s(e, h),
            {
              basename: y
            } = o.useContext(r.yo),
            g = !1;
          if ("string" == typeof p && D.test(p) && (n = p, F)) try {
            let e = new URL(window.location.href),
              t = p.startsWith("//") ? new URL(e.protocol + p) : new URL(p),
              n = (0, a.mc)(t.pathname, y);
            t.origin === e.origin && null != n ? p = n + t.search + t.hash : g = !0
          } catch (e) {}
          let E = (0, r.co)(p, {
              relative: u
            }),
            S = K(p, {
              replace: d,
              state: f,
              target: m,
              preventScrollReset: v,
              relative: u,
              unstable_viewTransition: w
            });
          return o.createElement("a", i({}, b, {
            href: n || E,
            onClick: g || c ? l : function(e) {
              l && l(e), e.defaultPrevented || S(e)
            },
            ref: t,
            target: m
          }))
        })),
        U = o.forwardRef((function(e, t) {
          let {
            "aria-current": n = "page",
            caseSensitive: a = !1,
            className: l = "",
            end: u = !1,
            style: c,
            to: d,
            unstable_viewTransition: f,
            children: m
          } = e, p = s(e, w), v = (0, r.oT)(d, {
            relative: p.relative
          }), h = (0, r.IT)(), b = o.useContext(r.Cu), {
            navigator: y
          } = o.useContext(r.yo), g = null != b && ne(v) && !0 === f, E = y.encodeLocation ? y.encodeLocation(v).pathname : v.pathname, S = h.pathname, R = b && b.navigation && b.navigation.location ? b.navigation.location.pathname : null;
          a || (S = S.toLowerCase(), R = R ? R.toLowerCase() : null, E = E.toLowerCase());
          let _, T = S === E || !u && S.startsWith(E) && "/" === S.charAt(E.length),
            C = null != R && (R === E || !u && R.startsWith(E) && "/" === R.charAt(E.length)),
            L = {
              isActive: T,
              isPending: C,
              isTransitioning: g
            },
            k = T ? n : void 0;
          _ = "function" == typeof l ? l(L) : [l, T ? "active" : null, C ? "pending" : null, g ? "transitioning" : null].filter(Boolean).join(" ");
          let x = "function" == typeof c ? c(L) : c;
          return o.createElement(N, i({}, p, {
            "aria-current": k,
            className: _,
            ref: t,
            style: x,
            to: d,
            unstable_viewTransition: f
          }), "function" == typeof m ? m(L) : m)
        })),
        P = o.forwardRef(((e, t) => {
          let n = B();
          return o.createElement(M, i({}, e, {
            submit: n,
            ref: t
          }))
        })),
        M = o.forwardRef(((e, t) => {
          let {
            reloadDocument: n,
            replace: r,
            state: a,
            method: u = l,
            action: c,
            onSubmit: d,
            submit: f,
            relative: m,
            preventScrollReset: p,
            unstable_viewTransition: v
          } = e, h = s(e, b), w = "get" === u.toLowerCase() ? "get" : "post", y = W(c, {
            relative: m
          });
          return o.createElement("form", i({
            ref: t,
            method: w,
            action: y,
            onSubmit: n ? d : e => {
              if (d && d(e), e.defaultPrevented) return;
              e.preventDefault();
              let t = e.nativeEvent.submitter,
                n = (null == t ? void 0 : t.getAttribute("formmethod")) || u;
              f(t || e.currentTarget, {
                method: n,
                replace: r,
                state: a,
                relative: m,
                preventScrollReset: p,
                unstable_viewTransition: v
              })
            }
          }, h))
        }));

      function I(e) {
        let {
          getKey: t,
          storageKey: n
        } = e;
        return $({
          getKey: t,
          storageKey: n
        }), null
      }
      var O, Y;

      function q(e) {
        let t = o.useContext(r.Mh);
        return t || (0, a.q4)(!1), t
      }

      function j(e) {
        let t = o.useContext(r.Cu);
        return t || (0, a.q4)(!1), t
      }

      function K(e, t) {
        let {
          target: n,
          replace: i,
          state: s,
          preventScrollReset: l,
          relative: u,
          unstable_viewTransition: c
        } = void 0 === t ? {} : t, d = (0, r.i6)(), f = (0, r.IT)(), m = (0, r.oT)(e, {
          relative: u
        });
        return o.useCallback((t => {
          if (function(e, t) {
              return !(0 !== e.button || t && "_self" !== t || function(e) {
                return !!(e.metaKey || e.altKey || e.ctrlKey || e.shiftKey)
              }(e))
            }(t, n)) {
            t.preventDefault();
            let n = void 0 !== i ? i : (0, a.Ep)(f) === (0, a.Ep)(m);
            d(e, {
              replace: n,
              state: s,
              preventScrollReset: l,
              relative: u,
              unstable_viewTransition: c
            })
          }
        }), [f, d, m, i, s, n, e, l, u, c])
      }

      function V(e) {
        let t = o.useRef(d(e)),
          n = o.useRef(!1),
          a = (0, r.IT)(),
          i = o.useMemo((() => function(e, t) {
            let n = d(e);
            return t && t.forEach(((e, o) => {
              n.has(o) || t.getAll(o).forEach((e => {
                n.append(o, e)
              }))
            })), n
          }(a.search, n.current ? null : t.current)), [a.search]),
          s = (0, r.i6)(),
          l = o.useCallback(((e, t) => {
            const o = d("function" == typeof e ? e(i) : e);
            n.current = !0, s("?" + o, t)
          }), [s, i]);
        return [i, l]
      }

      function z() {
        if ("undefined" == typeof document) throw new Error("You are calling submit during the server render. Try calling submit within a `useEffect` or callback instead.")
      }

      function B() {
        let {
          router: e
        } = q(O.UseSubmit), {
          basename: t
        } = o.useContext(r.yo), n = (0, r.WM)();
        return o.useCallback((function(o, r) {
          void 0 === r && (r = {}), z();
          let {
            action: a,
            method: i,
            encType: s,
            formData: l,
            body: u
          } = v(o, t);
          e.navigate(r.action || a, {
            preventScrollReset: r.preventScrollReset,
            formData: l,
            body: u,
            formMethod: r.method || i,
            formEncType: r.encType || s,
            replace: r.replace,
            state: r.state,
            fromRouteId: n,
            unstable_viewTransition: r.unstable_viewTransition
          })
        }), [e, t, n])
      }

      function H(e, t) {
        let {
          router: n
        } = q(O.UseSubmitFetcher), {
          basename: i
        } = o.useContext(r.yo);
        return o.useCallback((function(o, r) {
          void 0 === r && (r = {}), z();
          let {
            action: s,
            method: l,
            encType: u,
            formData: c,
            body: d
          } = v(o, i);
          null == t && (0, a.q4)(!1), n.fetch(e, t, r.action || s, {
            preventScrollReset: r.preventScrollReset,
            formData: c,
            body: d,
            formMethod: r.method || l,
            formEncType: r.encType || u
          })
        }), [n, i, e, t])
      }

      function W(e, t) {
        let {
          relative: n
        } = void 0 === t ? {} : t, {
          basename: s
        } = o.useContext(r.yo), l = o.useContext(r.EN);
        l || (0, a.q4)(!1);
        let [u] = l.matches.slice(-1), c = i({}, (0, r.oT)(e || ".", {
          relative: n
        })), d = (0, r.IT)();
        if (null == e && (c.search = d.search, u.route.index)) {
          let e = new URLSearchParams(c.search);
          e.delete("index"), c.search = e.toString() ? "?" + e.toString() : ""
        }
        return e && "." !== e || !u.route.index || (c.search = c.search ? c.search.replace(/^\?/, "?index&") : "?index"), "/" !== s && (c.pathname = "/" === c.pathname ? s : (0, a.En)([s, c.pathname])), (0, a.Ep)(c)
      }(function(e) {
        e.UseScrollRestoration = "useScrollRestoration", e.UseSubmit = "useSubmit", e.UseSubmitFetcher = "useSubmitFetcher", e.UseFetcher = "useFetcher", e.useViewTransitionState = "useViewTransitionState"
      })(O || (O = {})),
      function(e) {
        e.UseFetchers = "useFetchers", e.UseScrollRestoration = "useScrollRestoration"
      }(Y || (Y = {}));
      let G = 0;

      function J() {
        var e;
        let {
          router: t
        } = q(O.UseFetcher), n = o.useContext(r.EN);
        n || (0, a.q4)(!1);
        let s = null == (e = n.matches[n.matches.length - 1]) ? void 0 : e.route.id;
        null == s && (0, a.q4)(!1);
        let [l] = o.useState((() => String(++G))), [u] = o.useState((() => (s || (0, a.q4)(!1), function(e, t) {
          return o.forwardRef(((n, r) => {
            let a = H(e, t);
            return o.createElement(M, i({}, n, {
              ref: r,
              submit: a
            }))
          }))
        }(l, s)))), [c] = o.useState((() => e => {
          t || (0, a.q4)(!1), s || (0, a.q4)(!1), t.fetch(l, s, e)
        })), d = H(l, s), f = t.getFetcher(l), m = o.useMemo((() => i({
          Form: u,
          submit: d,
          load: c
        }, f)), [f, u, d, c]);
        return o.useEffect((() => () => {
          t ? t.deleteFetcher(l) : console.warn("No router available to clean up from useFetcher()")
        }), [t, l]), m
      }

      function X() {
        return [...j(Y.UseFetchers).fetchers.values()]
      }
      const Q = "react-router-scroll-positions";
      let Z = {};

      function $(e) {
        let {
          getKey: t,
          storageKey: n
        } = void 0 === e ? {} : e, {
          router: s
        } = q(O.UseScrollRestoration), {
          restoreScrollPosition: l,
          preventScrollReset: u
        } = j(Y.UseScrollRestoration), {
          basename: c
        } = o.useContext(r.yo), d = (0, r.IT)(), f = (0, r.y8)(), m = (0, r.Yi)();
        o.useEffect((() => (window.history.scrollRestoration = "manual", () => {
            window.history.scrollRestoration = "auto"
          })), []),
          function(e, t) {
            let {
              capture: n
            } = {};
            o.useEffect((() => {
              let t = null != n ? {
                capture: n
              } : void 0;
              return window.addEventListener("pagehide", e, t), () => {
                window.removeEventListener("pagehide", e, t)
              }
            }), [e, n])
          }(o.useCallback((() => {
            if ("idle" === m.state) {
              let e = (t ? t(d, f) : null) || d.key;
              Z[e] = window.scrollY
            }
            try {
              sessionStorage.setItem(n || Q, JSON.stringify(Z))
            } catch (e) {}
            window.history.scrollRestoration = "auto"
          }), [n, t, m.state, d, f])), "undefined" != typeof document && (o.useLayoutEffect((() => {
            try {
              let e = sessionStorage.getItem(n || Q);
              e && (Z = JSON.parse(e))
            } catch (e) {}
          }), [n]), o.useLayoutEffect((() => {
            let e = t && "/" !== c ? (e, n) => t(i({}, e, {
                pathname: (0, a.mc)(e.pathname, c) || e.pathname
              }), n) : t,
              n = null == s ? void 0 : s.enableScrollRestoration(Z, (() => window.scrollY), e);
            return () => n && n()
          }), [s, c, t]), o.useLayoutEffect((() => {
            if (!1 !== l)
              if ("number" != typeof l) {
                if (d.hash) {
                  let e = document.getElementById(decodeURIComponent(d.hash.slice(1)));
                  if (e) return void e.scrollIntoView()
                }!0 !== u && window.scrollTo(0, 0)
              } else window.scrollTo(0, l)
          }), [d, l, u]))
      }

      function ee(e, t) {
        let {
          capture: n
        } = t || {};
        o.useEffect((() => {
          let t = null != n ? {
            capture: n
          } : void 0;
          return window.addEventListener("beforeunload", e, t), () => {
            window.removeEventListener("beforeunload", e, t)
          }
        }), [e, n])
      }

      function te(e) {
        let {
          when: t,
          message: n
        } = e, a = (0, r.C5)(t);
        o.useEffect((() => {
          "blocked" === a.state && (window.confirm(n) ? setTimeout(a.proceed, 0) : a.reset())
        }), [a, n]), o.useEffect((() => {
          "blocked" !== a.state || t || a.reset()
        }), [a, t])
      }

      function ne(e, t) {
        void 0 === t && (t = {});
        let n = o.useContext(R);
        null == n && (0, a.q4)(!1);
        let {
          basename: i
        } = q(O.useViewTransitionState), s = (0, r.oT)(e, {
          relative: t.relative
        });
        if (!n.isTransitioning) return !1;
        let l = (0, a.mc)(n.currentLocation.pathname, i) || n.currentLocation.pathname,
          u = (0, a.mc)(n.nextLocation.pathname, i) || n.nextLocation.pathname;
        return null != (0, a.ot)(s.pathname, u) || null != (0, a.ot)(s.pathname, l)
      }
    }
  }
]);