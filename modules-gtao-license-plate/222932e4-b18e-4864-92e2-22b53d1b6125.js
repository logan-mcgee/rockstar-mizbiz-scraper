  ...
  [6801], {
    76801: (e, t, a) => {
      ...
      function d(e, t, a, n, i, f) {
        for (var l, d = e.data, s = 2 * f + 1, g = n - 1, b = i - 1, c = f + 1, y = c * (c + 1) / 2, p = new u, v = p, h = 1; h < s; h++) v = v.next = new u, h === c && (l = v);
        v.next = p;
        ...
        for (var oe = 0; oe < n; oe++) {
          ...
          for (var me = n, we = 0, xe = 0, _e = 0, Ee = 0, Ie = 1; Ie <= f; Ie++) {
            _ = me + oe << 2;
            var ke = c - Ie;
            ce += (v.r = ie = d[_]) * ke, ye += (v.g = fe = d[_ + 1]) * ke, pe += (v.b = le = d[_ + 2]) * ke, ve += (v.a = de = d[_ + 3]) * ke, Ee += ie, we += fe, xe += le, _e += de, v = v.next, Ie < b && (me += n)
          }
          _ = oe, m = p, w = l;
          for (var Ce = 0; Ce < i; Ce++) {
            var Be = _ << 2;
            d[Be + 3] = de = ve * E >>> I, de > 0 ? (de = 255 / de, d[Be] = (ce * E >>> I) * de, d[Be + 1] = (ye * E >>> I) * de, d[Be + 2] = (pe * E >>> I) * de) : d[Be] = d[Be + 1] = d[Be + 2] = 0, ce -= se, ye -= ge, pe -= ue, ve -= be, se -= m.r, ge -= m.g, ue -= m.b, be -= m.a, Be = oe + ((Be = Ce + c) < b ? Be : b) * n << 2, ce += Ee += m.r = d[Be], ye += we += m.g = d[Be + 1], pe += xe += m.b = d[Be + 2], ve += _e += m.a = d[Be + 3], m = m.next, se += ie = w.r, ge += fe = w.g, ue += le = w.b, be += de = w.a, Ee -= ie, we -= fe, xe -= le, _e -= de, w = w.next, _ += n
          }
        }
        return e
      }
      ...
      function g(e, t, a, n, i, f) {
        ...
        for (var z = 0; z < n; z++) {
          ...
          for (var ee = 0, te = 0, ae = 0, ne = 1, re = n; ne <= f; ne++) C = re + z << 2, V += (v.r = F = d[C]) * (w = c - ne), X += (v.g = J = d[C + 1]) * w, Z += (v.b = K = d[C + 2]) * w, ee += F, te += J, ae += K, v = v.next, ne < b && (re += n);
          C = z, x = p, _ = l;
          for (var oe = 0; oe < i; oe++) d[m = C << 2] = V * E >>> I, d[m + 1] = X * E >>> I, d[m + 2] = Z * E >>> I, V -= P, X -= Q, Z -= U, P -= x.r, Q -= x.g, U -= x.b, m = z + ((m = oe + c) < b ? m : b) * n << 2, V += ee += x.r = d[m], X += te += x.g = d[m + 1], Z += ae += x.b = d[m + 2], x = x.next, P += F = _.r, Q += J = _.g, U += K = _.b, ee -= F, te -= J, ae -= K, _ = _.next, C += n
        }
        return e
      }
      var u = function e() {
