  ...
  [6801], {
    76801: (e, t, a) => {
      ...
      function d(e, t, a, n, i, l) {
        for (var f, d = e.data, s = 2 * l + 1, g = n - 1, u = i - 1, c = l + 1, y = c * (c + 1) / 2, p = new b, v = p, h = 1; h < s; h++) v = v.next = new b, h === c && (f = v);
        v.next = p;
        ...
        for (var oe = 0; oe < n; oe++) {
          var ie = d[_ = oe << 2],
            ...
            ve = y * de;
          v = p;
          for (var he = 0; he < c; he++) v.r = ie, v.g = le, v.b = fe, v.a = de, v = v.next;
          for (var me = n, we = 0, xe = 0, _e = 0, Ee = 0, Ie = 1; Ie <= l; Ie++) {
            _ = me + oe << 2;
            var ke = c - Ie;
            ce += (v.r = ie = d[_]) * ke, ye += (v.g = le = d[_ + 1]) * ke, pe += (v.b = fe = d[_ + 2]) * ke, ve += (v.a = de = d[_ + 3]) * ke, Ee += ie, we += le, xe += fe, _e += de, v = v.next, Ie < u && (me += n)
          }
          _ = oe, m = p, w = f;
          for (var Ce = 0; Ce < i; Ce++) {
            var Be = _ << 2;
            d[Be + 3] = de = ve * E >>> I, de > 0 ? (de = 255 / de, d[Be] = (ce * E >>> I) * de, d[Be + 1] = (ye * E >>> I) * de, d[Be + 2] = (pe * E >>> I) * de) : d[Be] = d[Be + 1] = d[Be + 2] = 0, ce -= se, ye -= ge, pe -= be, ve -= ue, se -= m.r, ge -= m.g, be -= m.b, ue -= m.a, Be = oe + ((Be = Ce + c) < u ? Be : u) * n << 2, ce += Ee += m.r = d[Be], ye += we += m.g = d[Be + 1], pe += xe += m.b = d[Be + 2], ve += _e += m.a = d[Be + 3], m = m.next, se += ie = w.r, ge += le = w.g, be += fe = w.b, ue += de = w.a, Ee -= ie, we -= le, xe -= fe, _e -= de, w = w.next, _ += n
          }
        }
        return e
      }
      ...
      function g(e, t, a, n, i, l) {
        ...
        for (var z = 0; z < n; z++) {
          ...
          for (var ee = 0, te = 0, ae = 0, ne = 1, re = n; ne <= l; ne++) C = re + z << 2, V += (v.r = F = d[C]) * (w = c - ne), X += (v.g = J = d[C + 1]) * w, Z += (v.b = K = d[C + 2]) * w, ee += F, te += J, ae += K, v = v.next, ne < u && (re += n);
          C = z, x = p, _ = f;
