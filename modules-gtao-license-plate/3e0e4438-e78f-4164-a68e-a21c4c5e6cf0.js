  ...
  [6374], {
    36374: (e, t, a) => {
      ...
      function d(e, t, a, n, f, i) {
        for (var l, d = e.data, s = 2 * i + 1, g = n - 1, u = f - 1, b = i + 1, y = b * (b + 1) / 2, p = new c, v = p, h = 1; h < s; h++) v = v.next = new c, h === b && (l = v);
        v.next = p;
        ...
        for (var oe = 0; oe < n; oe++) {
          var fe = d[_ = oe << 2],
            ...
            ve = y * de;
          v = p;
          for (var he = 0; he < b; he++) v.r = fe, v.g = ie, v.b = le, v.a = de, v = v.next;
          for (var me = n, we = 0, xe = 0, _e = 0, Ee = 0, Ie = 1; Ie <= i; Ie++) {
            _ = me + oe << 2;
            var ke = b - Ie;
            be += (v.r = fe = d[_]) * ke, ye += (v.g = ie = d[_ + 1]) * ke, pe += (v.b = le = d[_ + 2]) * ke, ve += (v.a = de = d[_ + 3]) * ke, Ee += fe, we += ie, xe += le, _e += de, v = v.next, Ie < u && (me += n)
          }
          _ = oe, m = p, w = l;
          for (var Ce = 0; Ce < f; Ce++) {
            var Be = _ << 2;
            d[Be + 3] = de = ve * E >>> I, de > 0 ? (de = 255 / de, d[Be] = (be * E >>> I) * de, d[Be + 1] = (ye * E >>> I) * de, d[Be + 2] = (pe * E >>> I) * de) : d[Be] = d[Be + 1] = d[Be + 2] = 0, be -= se, ye -= ge, pe -= ce, ve -= ue, se -= m.r, ge -= m.g, ce -= m.b, ue -= m.a, Be = oe + ((Be = Ce + b) < u ? Be : u) * n << 2, be += Ee += m.r = d[Be], ye += we += m.g = d[Be + 1], pe += xe += m.b = d[Be + 2], ve += _e += m.a = d[Be + 3], m = m.next, se += fe = w.r, ge += ie = w.g, ce += le = w.b, ue += de = w.a, Ee -= fe, we -= ie, xe -= le, _e -= de, w = w.next, _ += n
          }
        }
        return e
      }
      ...
      function g(e, t, a, n, f, i) {
        ...
        for (var z = 0; z < n; z++) {
          ...
          for (var ee = 0, te = 0, ae = 0, ne = 1, re = n; ne <= i; ne++) C = re + z << 2, V += (v.r = F = d[C]) * (w = b - ne), X += (v.g = J = d[C + 1]) * w, Z += (v.b = K = d[C + 2]) * w, ee += F, te += J, ae += K, v = v.next, ne < u && (re += n);
          C = z, x = p, _ = l;
          for (var oe = 0; oe < f; oe++) d[m = C << 2] = V * E >>> I, d[m + 1] = X * E >>> I, d[m + 2] = Z * E >>> I, V -= P, X -= Q, Z -= U, P -= x.r, Q -= x.g, U -= x.b, m = z + ((m = oe + b) < u ? m : u) * n << 2, V += ee += x.r = d[m], X += te += x.g = d[m + 1], Z += ae += x.b = d[m + 2], x = x.next, P += F = _.r, Q += J = _.g, U += K = _.b, ee -= F, te -= J, ae -= K, _ = _.next, C += n
        }
        return e
      }
      var c = function e() {
