  ...
  [4023], {
    24023: (e, t, a) => {
      ...
      function g(e, t, a, n, f, l) {
        for (var i, g = e.data, s = 2 * l + 1, d = n - 1, u = f - 1, b = l + 1, y = b * (b + 1) / 2, v = new c, p = v, x = 1; x < s; x++) p = p.next = new c, x === b && (i = p);
        p.next = v;
        ...
        for (var oe = 0; oe < n; oe++) {
          ...
          for (var we = n, me = 0, he = 0, _e = 0, Ee = 0, Ie = 1; Ie <= l; Ie++) {
            _ = we + oe << 2;
            var ke = b - Ie;
            be += (p.r = fe = g[_]) * ke, ye += (p.g = le = g[_ + 1]) * ke, ve += (p.b = ie = g[_ + 2]) * ke, pe += (p.a = ge = g[_ + 3]) * ke, Ee += fe, me += le, he += ie, _e += ge, p = p.next, Ie < u && (we += n)
          }
          _ = oe, w = v, m = i;
          for (var Ce = 0; Ce < f; Ce++) {
            var Be = _ << 2;
            g[Be + 3] = ge = pe * E >> I, ge > 0 ? (ge = 255 / ge, g[Be] = (be * E >> I) * ge, g[Be + 1] = (ye * E >> I) * ge, g[Be + 2] = (ve * E >> I) * ge) : g[Be] = g[Be + 1] = g[Be + 2] = 0, be -= se, ye -= de, ve -= ce, pe -= ue, se -= w.r, de -= w.g, ce -= w.b, ue -= w.a, Be = oe + ((Be = Ce + b) < u ? Be : u) * n << 2, be += Ee += w.r = g[Be], ye += me += w.g = g[Be + 1], ve += he += w.b = g[Be + 2], pe += _e += w.a = g[Be + 3], w = w.next, se += fe = m.r, de += le = m.g, ce += ie = m.b, ue += ge = m.a, Ee -= fe, me -= le, he -= ie, _e -= ge, m = m.next, _ += n
          }
        }
        return e
      }
      ...
      function d(e, t, a, n, f, l) {
        ...
        for (var F = 0; F < n; F++) {
          ...
          for (var ee = 0, te = 0, ae = 0, ne = 1, re = n; ne <= l; ne++) C = re + F << 2, V += (p.r = J = g[C]) * (m = b - ne), X += (p.g = K = g[C + 1]) * m, Z += (p.b = O = g[C + 2]) * m, ee += J, te += K, ae += O, p = p.next, ne < u && (re += n);
          C = F, h = v, _ = i;
          for (var oe = 0; oe < f; oe++) g[w = C << 2] = V * E >> I, g[w + 1] = X * E >> I, g[w + 2] = Z * E >> I, V -= P, X -= Q, Z -= U, P -= h.r, Q -= h.g, U -= h.b, w = F + ((w = oe + b) < u ? w : u) * n << 2, V += ee += h.r = g[w], X += te += h.g = g[w + 1], Z += ae += h.b = g[w + 2], h = h.next, P += J = _.r, Q += K = _.g, U += O = _.b, ee -= J, te -= K, ae -= O, _ = _.next, C += n
        }
        return e
      }
      var c = function e() {
