  ...
  [6801], {
    76801: (e, t, a) => {
      ...
      function s(e, t, a, n, i, f) {
        for (var l, s = e.data, d = 2 * f + 1, g = n - 1, b = i - 1, u = f + 1, y = u * (u + 1) / 2, p = new c, v = p, h = 1; h < d; h++) v = v.next = new c, h === u && (l = v);
        v.next = p;
        ...
        for (var oe = 0; oe < n; oe++) {
          ...
          for (var me = n, we = 0, xe = 0, _e = 0, Ee = 0, Ie = 1; Ie <= f; Ie++) {
            _ = me + oe << 2;
            var ke = u - Ie;
            ue += (v.r = ie = s[_]) * ke, ye += (v.g = fe = s[_ + 1]) * ke, pe += (v.b = le = s[_ + 2]) * ke, ve += (v.a = se = s[_ + 3]) * ke, Ee += ie, we += fe, xe += le, _e += se, v = v.next, Ie < b && (me += n)
          }
          _ = oe, m = p, w = l;
          for (var Ce = 0; Ce < i; Ce++) {
            var Be = _ << 2;
            s[Be + 3] = se = ve * E >>> I, se > 0 ? (se = 255 / se, s[Be] = (ue * E >>> I) * se, s[Be + 1] = (ye * E >>> I) * se, s[Be + 2] = (pe * E >>> I) * se) : s[Be] = s[Be + 1] = s[Be + 2] = 0, ue -= de, ye -= ge, pe -= ce, ve -= be, de -= m.r, ge -= m.g, ce -= m.b, be -= m.a, Be = oe + ((Be = Ce + u) < b ? Be : b) * n << 2, ue += Ee += m.r = s[Be], ye += we += m.g = s[Be + 1], pe += xe += m.b = s[Be + 2], ve += _e += m.a = s[Be + 3], m = m.next, de += ie = w.r, ge += fe = w.g, ce += le = w.b, be += se = w.a, Ee -= ie, we -= fe, xe -= le, _e -= se, w = w.next, _ += n
          }
        }
        return e
      }
      ...
      function g(e, t, a, n, i, f) {
        ...
        for (var m, w, x = null, _ = null, E = r[f], I = o[f], k = 0, C = 0, B = 0; B < i; B++) {
          ...
          for (var O = 0, W = 0, L = 0, Y = 1; Y < u; Y++) m = C + ((g < Y ? g : Y) << 2), G += (v.r = S = s[m]) * (w = u - Y), j += (v.g = T = s[m + 1]) * w, A += (v.b = D = s[m + 2]) * w, O += S, W += T, L += D, v = v.next;
          x = p, _ = l;
        ...
        for (var z = 0; z < n; z++) {
          ...
          for (var ee = 0, te = 0, ae = 0, ne = 1, re = n; ne <= f; ne++) C = re + z << 2, V += (v.r = F = s[C]) * (w = u - ne), X += (v.g = J = s[C + 1]) * w, Z += (v.b = K = s[C + 2]) * w, ee += F, te += J, ae += K, v = v.next, ne < b && (re += n);
          C = z, x = p, _ = l;
          for (var oe = 0; oe < i; oe++) s[m = C << 2] = V * E >>> I, s[m + 1] = X * E >>> I, s[m + 2] = Z * E >>> I, V -= P, X -= Q, Z -= U, P -= x.r, Q -= x.g, U -= x.b, m = z + ((m = oe + u) < b ? m : b) * n << 2, V += ee += x.r = s[m], X += te += x.g = s[m + 1], Z += ae += x.b = s[m + 2], x = x.next, P += F = _.r, Q += J = _.g, U += K = _.b, ee -= F, te -= J, ae -= K, _ = _.next, C += n
        }
        return e
      }
      var c = function e() {
