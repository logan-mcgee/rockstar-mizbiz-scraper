try {
  ...
  [6801], {
    76801: (e, t, a) => {
      ...
      function f(e, t, a, n, r, o) {
        if ("string" == typeof e && (e = document.getElementById(e)), e && ("HTMLImageElement" !== Object.prototype.toString.call(e).slice(8, -1) || "naturalWidth" in e)) {
          var f = r ? "offset" : "natural",
            i = e[f + "Width"],
            s = e[f + "Height"];
          if ("ImageBitmap" === Object.prototype.toString.call(e).slice(8, -1) && (i = e.width, s = e.height), "string" == typeof t && (t = document.getElementById(t)), t && "getContext" in t) {
            o || (t.style.width = i + "px", t.style.height = s + "px"), t.width = i, t.height = s;
            var g = t.getContext("2d");
            g.clearRect(0, 0, i, s), g.drawImage(e, 0, 0, e.naturalWidth, e.naturalHeight, 0, 0, i, s), isNaN(a) || a < 1 || (n ? l(t, 0, 0, i, s, a) : d(t, 0, 0, i, s, a))
          }
        }
      }
      ...
      function s(e, t, a, n, f, i) {
        for (var l, s = e.data, d = 2 * i + 1, g = n - 1, u = f - 1, b = i + 1, y = b * (b + 1) / 2, p = new c, v = p, h = 1; h < d; h++) v = v.next = new c, h === b && (l = v);
        v.next = p;
        for (var m = null, w = null, x = 0, _ = 0, E = r[i], I = o[i], k = 0; k < f; k++) {
          v = p;
        ...
        for (var oe = 0; oe < n; oe++) {
          var fe = s[_ = oe << 2],
            ...
            ve = y * se;
          v = p;
          for (var he = 0; he < b; he++) v.r = fe, v.g = ie, v.b = le, v.a = se, v = v.next;
          for (var me = n, we = 0, xe = 0, _e = 0, Ee = 0, Ie = 1; Ie <= i; Ie++) {
            _ = me + oe << 2;
            var ke = b - Ie;
            be += (v.r = fe = s[_]) * ke, ye += (v.g = ie = s[_ + 1]) * ke, pe += (v.b = le = s[_ + 2]) * ke, ve += (v.a = se = s[_ + 3]) * ke, Ee += fe, we += ie, xe += le, _e += se, v = v.next, Ie < u && (me += n)
          }
          _ = oe, m = p, w = l;
          for (var Ce = 0; Ce < f; Ce++) {
            var Be = _ << 2;
            s[Be + 3] = se = ve * E >>> I, se > 0 ? (se = 255 / se, s[Be] = (be * E >>> I) * se, s[Be + 1] = (ye * E >>> I) * se, s[Be + 2] = (pe * E >>> I) * se) : s[Be] = s[Be + 1] = s[Be + 2] = 0, be -= de, ye -= ge, pe -= ce, ve -= ue, de -= m.r, ge -= m.g, ce -= m.b, ue -= m.a, Be = oe + ((Be = Ce + b) < u ? Be : u) * n << 2, be += Ee += m.r = s[Be], ye += we += m.g = s[Be + 1], pe += xe += m.b = s[Be + 2], ve += _e += m.a = s[Be + 3], m = m.next, de += fe = w.r, ge += ie = w.g, ce += le = w.b, ue += se = w.a, Ee -= fe, we -= ie, xe -= le, _e -= se, w = w.next, _ += n
          }
        }
        return e
      }
      ...
      function g(e, t, a, n, f, i) {
        for (var l, s = e.data, d = 2 * i + 1, g = n - 1, u = f - 1, b = i + 1, y = b * (b + 1) / 2, p = new c, v = p, h = 1; h < d; h++) v = v.next = new c, h === b && (l = v);
        v.next = p;
        for (var m, w, x = null, _ = null, E = r[i], I = o[i], k = 0, C = 0, B = 0; B < f; B++) {
          var S = s[C],
            T = s[C + 1],
            D = s[C + 2],
            M = b * S,
          ...
          x = p, _ = l;
          for (var q = 0; q < n; q++) s[C] = G * E >>> I, s[C + 1] = j * E >>> I, s[C + 2] = A * E >>> I, G -= M, j -= N, A -= R, M -= x.r, N -= x.g, R -= x.b, m = k + ((m = q + i + 1) < g ? m : g) << 2, G += O += x.r = s[m], j += W += x.g = s[m + 1], A += L += x.b = s[m + 2], x = x.next, M += S = _.r, N += T = _.g, R += D = _.b, O -= S, W -= T, L -= D, _ = _.next, C += 4;
          k += n
        }
        for (var z = 0; z < n; z++) {
          var F = s[C = z << 2],
          ...
          for (var ee = 0, te = 0, ae = 0, ne = 1, re = n; ne <= i; ne++) C = re + z << 2, V += (v.r = F = s[C]) * (w = b - ne), X += (v.g = J = s[C + 1]) * w, Z += (v.b = K = s[C + 2]) * w, ee += F, te += J, ae += K, v = v.next, ne < u && (re += n);
          C = z, x = p, _ = l;
          for (var oe = 0; oe < f; oe++) s[m = C << 2] = V * E >>> I, s[m + 1] = X * E >>> I, s[m + 2] = Z * E >>> I, V -= P, X -= Q, Z -= U, P -= x.r, Q -= x.g, U -= x.b, m = z + ((m = oe + b) < u ? m : u) * n << 2, V += ee += x.r = s[m], X += te += x.g = s[m + 1], Z += ae += x.b = s[m + 2], x = x.next, P += F = _.r, Q += J = _.g, U += K = _.b, ee -= F, te -= J, ae -= K, _ = _.next, C += n
        }
        return e
      }
      var c = function e() {
